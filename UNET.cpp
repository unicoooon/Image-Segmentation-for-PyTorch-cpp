...
class miniNet:public torch::nn::Module
{
    public:
        ...
        resnet50<torch::Tensor> *bc = new resnet50<torch::Tensor>(256,64,mp);
...
template<typename T>
miniNet<T>::miniNet(int in_ch,int out_ch,std::unordered_map<std::string,std::vector<int>> chs):inch(in_ch),outch(out_ch),mp(chs),
                                          ...
                                          fl(torch::nn::Conv2dOptions(64,10,3).padding(1))
{
...
int main()
{
    ...
    mp["conv11"]={64,64};
    mp["conn"]={64,64}; 
    miniNet<torch::Tensor> net(1,10,mp);
    trainModel(net);
    predict(net);
    return 0;
}
